...
bool SocketConnecter::connect(const std::string& ip, size_t port)
{
	...
	if (socket_ == INVALID_SOCKET) {
		int error = WSAGetLastError();
		Verbose::show("\n  -- unable to connect to other peer, error = " + toString(error));
		return false;
	}
	return true;
}
...
{
	Verbose::show("\n Message # ==================================> SocketListener instance destroyed");
}
...
bool SocketListener::bind()
{
	...
	for (auto pResult = result; pResult != NULL; pResult = pResult->ai_next)
	{
		...
		Verbose::show("Message # ==================================> This peer created ListenSocket \n");
...
bool SocketListener::listen()
{
	...
	Verbose::show("\n  Message # =====================> This peer TCP listening socket setup complete");
	return true;
}
...
Socket SocketListener::accept()
{
	...
	if (!clientSocket.validState()) {
		acceptFailed_ = true;
		int error = WSAGetLastError();
		Verbose::show("\n Message # ==========================> Accept failed with error: " + toString(error), always);
		return clientSocket;
	}
	return clientSocket;
}
...
void ClientHandler::operator()(Socket& socket_)
{
	...
	Verbose::show("Message # ==================================> ClientHandler socket connection closing");
	socket_.shutDown();
	socket_.close();
	Verbose::show("Message # ==================================> ClientHandler thread terminating");
}
